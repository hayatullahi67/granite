...
export const MySales: React.FC = () => {
    ...
    const adminUserStats = useMemo(() => {
        ...
        transactions.forEach(t => {
            ...
            group.totalRevenue += (Number(t.totalInvoice) || 0);
            group.totalVolume += (Number(t.quantity) || 0);
            group.txCount += 1;

            const primaryProduct = t.items?.[0]?.productName || 'Unknown';
            group.productStats[primaryProduct] = (group.productStats[primaryProduct] || 0) + (Number(t.totalInvoice) || 0);
        });
    ...
    const personalSalesData = useMemo(() => {
        ...
        myTx.forEach(t => {
            const pName = t.items?.[0]?.productName || 'Unknown';
            ...
            productStats[pName].quantity += (Number(t.quantity) || 0);
            productStats[pName].revenue += (Number(t.totalInvoice) || 0);
    ...
    const totals = useMemo(() => {
        return transactions.reduce((acc, t) => {
            ...
            if (isMyTx) {
                acc.qty += (Number(t.quantity) || 0);
                acc.rev += (Number(t.totalInvoice) || 0);
                acc.count += 1;
            }
            return acc;
        }, { qty: 0, rev: 0, count: 0 });
